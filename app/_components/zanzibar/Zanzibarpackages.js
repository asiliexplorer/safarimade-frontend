...
import { useGetPackagesQuery } from "../../redux/features/admin/PackageApi";
...
const infoCards = [
  {
    ...
    description:
      "Zanzibar offers pristine beaches, historic Stone Town, spice plantations, and vibrant marine life. Perfect for combining relaxation with cultural immersion and adventure.",
  },
  {
    title: "Best time to visit",
    description:
      "June to October is the dry season with calm seas and best weather. December to February offers warm temperatures and coral spawning events. Avoid April and May due to heavy rainfall.",
  },
  {
    title: "What to expect",
    description:
      "Expect island hopping, snorkeling, dhow sailing, beach relaxation, and cultural tours. Zanzibar blends adventure with tranquility, perfect for honeymooners and families alike.",
  },
];

const faqItems = [
  {
    question: "How many days do I need in Zanzibar?",
    answer:
      "3-5 days is ideal for a relaxing beach getaway. Combine with a safari for a complete Tanzania experience with 8-12 days total.",
  },
  {
    question: "What activities are available?",
    answer:
      "Snorkeling, scuba diving, island tours, spice plantation visits, Stone Town exploration, dhow sunset cruises, and beach relaxation.",
  },
  {
    question: "Can I combine Zanzibar with a safari?",
    answer:
      "Yes. Many packages combine a northern safari (3-4 days) with Zanzibar beach time (3-5 days) for a complete Tanzania experience.",
  },
  {
    question: "Is Zanzibar good for families?",
    answer:
      "Absolutely. Most resorts and lodges are family-friendly with calm beach areas, kids activities, and cultural experiences suitable for all ages.",
  },
];
...
const PackageCard = ({ pkg }) => {
  ...
  const detailsHref = `/zanzibar-tour-packages/${pkg.slug || slugifyPackageName(pkg.name)}`;
    ...
    <article className="group flex h-full flex-col overflow-hidden rounded-[1.5rem] border border-gray-100 bg-white shadow-[0_10px_34px_rgba(17,24,39,0.08)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_18px_48px_rgba(17,24,39,0.14)]">
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <p className="mt-3 line-clamp-2 text-sm leading-6 text-gray-600">
          {pkg.shortDescription || pkg.experienceSummary || pkg.fullDescription || "Discover one of Zanzibar's standout beach journeys."}
        </p>
...
const ZanzibarPackages = () => {
  ...
  const { data: packagesData, isLoading, isError } = useGetPackagesQuery({
    category: "ZANZIBAR",
    limit: 100,
  });
  ...
  // Extract and normalize packages from Redux response
  useEffect(() => {
    if (packagesData?.data?.items) {
      const items = Array.isArray(packagesData.data.items) ? packagesData.data.items : [];
  ...
  const minDuration = useMemo(
    () => Math.min(...backendPackages.map((pkg) => Number(pkg.duration) || 0).filter(Boolean), 1),
    [backendPackages],
  );
  const maxDuration = useMemo(
    () => Math.max(...backendPackages.map((pkg) => Number(pkg.duration) || 0).filter(Boolean), 21),
    [backendPackages],
  );
  const minPrice = useMemo(
    () => Math.min(...backendPackages.map((pkg) => Number(pkg.priceFrom ?? pkg.price) || 0).filter(Boolean), 100),
    [backendPackages],
  );
  const maxPrice = useMemo(
    () => Math.max(...backendPackages.map((pkg) => Number(pkg.priceFrom ?? pkg.price) || 0).filter(Boolean), 10000),
    [backendPackages],
  );
  ...
  const filteredPackages = useMemo(() => {
    return allPackages.filter((pkg) => {
      ...
      const matchesStartingFrom =
        activeFilters.startingFrom === "all" ||
        startingFrom.includes(String(activeFilters.startingFrom).replace("-", " ").toLowerCase());
      const matchesComfortLevel =
        activeFilters.comfortLevel === "all" || comfortLevel.toLowerCase() === String(activeFilters.comfortLevel).toLowerCase();
      ...
      const matchesSpecializedTours =
        activeFilters.specializedTours.length === 0 ||
        activeFilters.specializedTours.every((tour) => haystack.includes(String(tour).replace(/-/g, " ").toLowerCase()));
      const matchesOtherFeatures =
        activeFilters.otherFeatures.length === 0 ||
      ...
      return (
        ...
        matchesTourType &&
        matchesSafariType &&
        matchesSpecializedTours &&
        matchesOtherFeatures
      );
    });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      {isLoading && backendPackages.length === 0 ? (
        ...
        <>
          <PackageHeroBanner
            ...
            titlePrefix="ZANZIBAR"
            titleHighlight="ESCAPES"
            subtitle="Pristine Beaches & Island Paradise"
            description="Explore Zanzibar's turquoise waters, rich heritage, and island adventures with our curated beach and safari combinations."
          />

          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-6">
            {isError && (
              <div className="mb-6 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                We encountered an error loading packages. Please try again later.
              </div>
            )}
            ...
            <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
                          ...
                          <div className="relative">
                            ...
                            <div
                              className={`w-5 h-5 border-2 rounded-full flex items-center justify-center transition-all duration-200 ${
                                activeFilters.startingFrom === option.id ? "border-[#8B6F47] bg-[#8B6F47]" : "border-gray-300 group-hover:border-[#8B6F47]"
                              }`}
                            >
                              {activeFilters.startingFrom === option.id && <div className="w-2 h-2 bg-white rounded-full" />}
                            </div>
                          </div>
                      ...
                      {filterOptions.comfortLevels.map((option, index) => (
                        <label key={`${option.id}-${index}`} className="flex items-center space-x-3 group cursor-pointer">
                          <div className="relative">
                            ...
                            <div
                              className={`w-5 h-5 border-2 rounded-full flex items-center justify-center transition-all duration-200 ${
                                activeFilters.comfortLevel === option.id ? "border-[#8B6F47] bg-[#8B6F47]" : "border-gray-300 group-hover:border-[#8B6F47]"
                              }`}
                            >
                              {activeFilters.comfortLevel === option.id && <div className="w-2 h-2 bg-white rounded-full" />}
                            </div>
                          </div>
                          <span className="text-gray-700 group-hover:text-gray-900 transition-colors">{option.name}</span>
                        </label>
                      ...
                      {filterOptions.tourTypes.map((option, index) => (
                        <label key={`${option.id}-${index}`} className="flex items-center space-x-3 group cursor-pointer">
                          <div className="relative">
                            ...
                            <div
                              className={`w-5 h-5 border-2 rounded-full flex items-center justify-center transition-all duration-200 ${
                                activeFilters.tourType === option.id ? "border-[#8B6F47] bg-[#8B6F47]" : "border-gray-300 group-hover:border-[#8B6F47]"
                              }`}
                            >
                              {activeFilters.tourType === option.id && <div className="w-2 h-2 bg-white rounded-full" />}
                            </div>
                          </div>
                          <span className="text-gray-700 group-hover:text-gray-900 transition-colors">{option.name}</span>
                        </label>
                      ...
                      {filterOptions.safariTypes.map((option, index) => (
                        <label key={`${option.id}-${index}`} className="flex items-center space-x-3 group cursor-pointer">
                          <div className="relative">
                            ...
                            <div
                              className={`w-5 h-5 border-2 rounded-full flex items-center justify-center transition-all duration-200 ${
                                activeFilters.safariType === option.id ? "border-[#8B6F47] bg-[#8B6F47]" : "border-gray-300 group-hover:border-[#8B6F47]"
                              }`}
                            >
                              {activeFilters.safariType === option.id && <div className="w-2 h-2 bg-white rounded-full" />}
                            </div>
                          </div>
                          <span className="text-gray-700 group-hover:text-gray-900 transition-colors">{option.name}</span>
                        </label>
                          ...
                          <div className="relative">
                            ...
                            <div
                              className={`w-5 h-5 border-2 rounded flex items-center justify-center transition-all duration-200 ${
                                activeFilters.specializedTours.includes(option.id) ? "border-[#8B6F47] bg-[#8B6F47]" : "border-gray-300 group-hover:border-[#8B6F47]"
                              }`}
                            >
                              {activeFilters.specializedTours.includes(option.id) && <FaCheck className="w-3 h-3 text-white" />}
                            </div>
                          </div>
                          <span className="text-gray-700 group-hover:text-gray-900 transition-colors">{option.name}</span>
                        </label>
                      ...
                      {filterOptions.otherFeatures.map((option, index) => (
                        <label key={`${option.id}-${index}`} className="flex items-center space-x-3 group cursor-pointer">
                          <div className="relative">
                            ...
                            <div
                              className={`w-5 h-5 border-2 rounded flex items-center justify-center transition-all duration-200 ${
                                activeFilters.otherFeatures.includes(option.id) ? "border-[#8B6F47] bg-[#8B6F47]" : "border-gray-300 group-hover:border-[#8B6F47]"
                              }`}
                            >
                              {activeFilters.otherFeatures.includes(option.id) && <FaCheck className="w-3 h-3 text-white" />}
                            </div>
                          </div>
                          <span className="text-gray-700 group-hover:text-gray-900 transition-colors">{option.name}</span>
                        </label>
              ...
              <div className="flex-1 min-w-0">
                <div className="mb-6 flex items-end justify-between gap-4">
                  <div>
                    <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#8B6F47] mb-2">Zanzibar Packages</h1>
                    <p className="text-sm sm:text-base text-gray-600 max-w-2xl">
                      Explore our curated selection of island escapes and beach experiences.
                    </p>
                  </div>
                ...
                {currentPagePackages.length > 0 ? (
                    ...
                    <div className="mt-10 flex items-center justify-center gap-2">
                      <button
                        ...
                        className={`inline-flex h-10 w-10 items-center justify-center rounded-full border transition-colors ${
                          currentPage === 1 ? "cursor-not-allowed border-gray-200 text-gray-300" : "border-gray-200 text-[#8B6F47] hover:bg-gray-100"
                        }`}
                        ...
                        return (
                          <button
                            ...
                            className={`h-10 w-10 rounded-full text-sm font-semibold transition-colors ${
                              isActive ? "bg-[#8B6F47] text-white" : "text-[#8B6F47] hover:bg-gray-100"
                            }`}
                      ...
                      <button
                        ...
                        className={`inline-flex h-10 w-10 items-center justify-center rounded-full border transition-colors ${
                          currentPage === totalPages ? "cursor-not-allowed border-gray-200 text-gray-300" : "border-gray-200 text-[#8B6F47] hover:bg-gray-100"
                        }`}
                  ...
                  <div className="rounded-3xl border border-gray-200 bg-white py-16 text-center shadow-lg">
                    ...
                    <p className="mx-auto mb-6 max-w-md text-gray-500">
                      We couldn't find any packages matching your search. Try a different search term or reset the filters.
                    </p>
                    <button
                      onClick={resetFilters}
            ...
            <section className="mt-8 space-y-10">
              <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-lg sm:p-8 lg:p-10">
                <div className="max-w-3xl">
                  <p className="text-sm font-semibold uppercase tracking-[0.25em] text-[#8B6F47]">Zanzibar Travel Guide</p>
                  <h2 className="mt-3 text-3xl font-bold text-gray-900">Plan your perfect island escape</h2>
                </div>
              ...
              <div className="rounded-3xl bg-white border border-gray-200 p-6 shadow-lg sm:p-8">
